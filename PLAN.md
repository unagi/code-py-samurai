...
## Current Workstream (Python API Reference / Learner Docs)

### 目的

- 学習者向け Python API 契約を先に固める
- 実装変更（大きい）とは分離してレビュー可能にする
- 最終的にゲーム画面へ「要約 + 詳細リファレンス（別タブ）」として戻す

### いま決まっている方針（重要）

- `docs/python-api-reference.md` は学習者向けの「公開契約」を記述する
- 実装都合（Skulpt 内部表現、互換のための受け入れ等）は原則として前面に出さない
- `Direction` / `Terrain` / `UnitKind` を使う
- `Space` は `terrain + unit` を持つセルとして説明する（`Occupant` を含むモデル）
- レベル表記はユーザーが画面で見る `Level` のみを使う（`GlobalLevel` を出さない）
- 実行エンジンは Skulpt のため、内部実装が CPython 的な理想形と少し異なる可能性はある
  - ただし公開契約（名前・意味・使い方）は守る

### PR分割方針（この順で進める）

1. Docs PR（現在のPR）
   - 範囲: `docs/python-api-reference.md`, `docs/rubywarrior-to-pythonic.md`
   - 目的: 学習者向け API 契約と設計理由を固定する
   - 禁止: エンジン/ランタイム/UI 実装変更を混ぜない
2. UI導線PR（小さめ）
   - 範囲: ゲーム画面に API 要約表示 + 詳細リンク（別タブ）追加
   - 目的: 学習者が今いる画面から参照導線を得られるようにする
   - まずは表示土台と遷移を優先（詳細内容の完全移植は次PRでも可）
3. 詳細リファレンスUI PR（本命）
   - 範囲: Javadoc風の詳細リファレンスページ（美しいレイアウト） + i18n対応
   - 目的: `python-api-reference` をゲーム内体験として読める形にする
   - 要件: 要約と詳細で情報密度を分ける（段階的開示）
4. 実装追随PR（別PR）
   - 範囲: `runtime/`, `engine/`, 必要に応じてレベルヒント文言
   - 目的: 仕様先行の `python-api-reference` に実行系を寄せる
   - 注意: 互換維持（既存プレイ/テスト）を確認しながら段階的に行う

### UIゴール（合意事項）

- ゲーム画面内には「今のレベルで使うAPIの要約」を表示する
- 詳細は別タブで開ける「ガチのリファレンス」ページを用意する
- 詳細ページは Javadoc 的な構造で、見た目も読み物として成立させる
- i18n対応する（少なくとも `ja` / `en`）

### 実装前に決めるべき設計タスク（UI/Docsデータ化）

1. リファレンスのソース・オブ・トゥルースを決める
   - 候補A: `docs/python-api-reference.md` を一次ソースにして UI 用に変換
   - 候補B: 構造化データ（TS/JSON）を一次ソースにして docs/UI を生成
   - 推奨: B（i18n と UI 表示に強い）
2. 要約に出す最小項目を定義する
   - 例: メソッド名 / 署名 / 一行説明 / `@since Level`
3. 詳細ページの情報粒度を定義する
   - 例: クラス説明 / プロパティ / メソッド / `@param` / `@return` / `@throws` / `@since`
4. i18nの分離方針を定義する
   - API識別子（`walk`, `Direction.LEFT` 等）は非翻訳
   - 説明文・タグ説明のみ翻訳
5. レベル連動表示のルールを定義する
   - 現在レベルまでに解放済み API のみ要約表示
   - 未解放APIは要約には出さない（詳細では「将来のAPI」として扱うか要検討）

### 再開用チェックリスト（Context解放後）

- 現在の docs PR は「ドキュメント限定」で維持する（実装変更を混ぜない）
- 参照ドキュメント:
  - `docs/python-api-reference.md`
  - `docs/rubywarrior-to-pythonic.md`
- 次PR候補:
  - UI導線PR（要約 + 詳細別タブリンク）
  - 詳細リファレンスUI PR（Javadoc風 + i18n）
  - 実装追随PR（`Space/Direction` モデル対応）
- 実装追随PRは Skulpt/ランタイム制約を前提に、公開契約を守る形で段階的に進める
